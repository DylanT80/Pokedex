...
// Fills the pokedex w/ pokemon info
...
// @param pokemon - data of the pokemon
const addPokemon = (pokemon) => {
    const li = document.createElement("li");
    pokemon.name = pokemon.name[0].toUpperCase() + pokemon.name.slice(1);   // Style name
...
// @params id   - unique identifier for pokemon
// @return json - a pokemon's data
...
input.addEventListener("keypress", (event) => {
    if (event.key === "Enter") {
        fillPokedex(event.target.value);
    }
});
